...
## 17. Canary safety review, 2026-08-03 — rollout held

The canary was **not run**. Before the first production write, the namespace
tooling was audited and found to have a partial-setup cleanup gap. The rollout
was held there, and the fix below is local code, tests and documentation only.

### 17.1 What the audit found

`ProductionCanaryNamespace.create()` performed every setup write — two branches,
two rooms, five stock locations, a category, an item, and four Auth identities
each with a domain user and an auth link — and only then returned the instance.
The harnesses' `try`/`finally` could therefore not call `retire()` until setup
had already succeeded in full. Four consequences, all blockers:

1. **A partial namespace had no owner.** A fault at any setup step threw before
   the caller ever held the object that knew what had been created, so the rows
   already on the server stayed active with nothing tracking them.
2. **A created Auth identity could go unrecorded.** The actor was appended to
   `actors` only after `createUser`, the `public.users` insert *and* the
   `user_auth_links` insert had all succeeded. A failure in either insert left a
   live production credential that cleanup could not see — the most serious of
   the four, because the leftover is a login rather than a row.
3. **`user_auth_links` was created but not accounted for.** It appeared in
   neither the retirement list nor the `left_in_place` report.
4. **Retirement counted intent, not effect.** `retired += entry.ids.length` ran
   whenever PostgREST returned no error, which proves the statement was accepted
   — not that any row matched or changed.

Two further findings, non-blocking but fixed here: the harness generated the
purchase-request line id and the opname-link id but tracked only the two parent
documents, and there was no local test that a partial setup cleans up.

### 17.2 Schema audit behind the policy

| Table | PK | `deleted_at` | `updated_at` | `is_active` | Retirement |
| --- | --- | --- | --- | --- | --- |
| `branches` | `id` | yes | yes | yes | soft-retire, exact ids |
| `rooms` | `id` | yes | yes | yes | soft-retire, exact ids |
| `users` | `id` | yes | yes | yes | soft-retire, exact ids |
| `items` | `id` | yes | yes | yes | soft-retire, exact ids |
| `stock_locations` | `id` | yes | yes | no | soft-retire, exact ids |
| `item_categories` | `id` | yes | yes | no | soft-retire, exact ids |
| `stock_opnames` | `id` | yes | yes | no | soft-retire, exact ids |
| `purchase_requests` | `id` | yes | yes | no | soft-retire, exact ids |
| `purchase_request_lines` | `id` | yes | yes | no | soft-retire, exact ids |
| `purchase_request_opnames` | `id` | yes | yes | no | soft-retire, exact ids |
| `user_auth_links` | **`auth_user_id`** | **no** | **no** | **no** | **left in place** |
| `sync_devices`, `sync_operations`, `sync_conflicts` | — | — | — | — | left in place, audit |
| `sync_change_journal`, `sync_entity_field_versions` | — | — | — | — | left in place, append-only |
| `stock_movements` | — | — | — | — | left in place, ledger |

No migration was added. `user_auth_links` is not defective — it is a join row
whose lifetime is the Auth identity's, deliberately cascading from
`auth.users`. Adding a `deleted_at` to it for the convenience of a canary would
be changing the product's schema to suit a test, which this task does not do.

### 17.3 `user_auth_links` policy

Policy **B**, explicitly: no soft-delete mechanism exists, so the row is
**never deleted and never updated**. It is enumerated in
`left_in_place_detail` with the masked `auth_user_id` and `user_id`, one entry
per actor. What makes this safe is that the credential it points at is
neutralised at the other end — every Auth identity the server confirmed is
banned for 876000h and its password rotated to a fresh random value, and the
domain user it points at is soft-retired with `deleted_at` and
`is_active = false`. The link therefore ties a banned login to a retired user.

### 17.4 Purchase-request child-row policy

`app_private.sync_submit_purchase_request` inserts `purchase_request_lines` and
`purchase_request_opnames` using `(line->>'id')::uuid` — the ids come from the
push payload, never from the server. The harness therefore knows the exact
primary key of every row the push creates, and now tracks all four:
`stock_opnames`, `purchase_requests`, `purchase_request_lines`,
`purchase_request_opnames`. All four soft-retire by exact id. Nothing is
inferred from cascade behaviour, because there is no soft-delete cascade to
infer from. `sync_operations` and `sync_change_journal` rows generated by the
push are left in place as audit, and named in the report.

### 17.5 What was changed

- `create()` builds the instance, and every deterministic id, **before** the
  first remote write. Setup runs inside a `try` whose `catch` retires the
  partial namespace best-effort and then throws an error carrying both the
  original fault and the cleanup outcome — `retirement_ok`, `retired/requested`,
  `auth_banned/created`, `left_in_place`, and any cleanup problems. A caller can
  never read a failed setup as a success.
- An Auth identity is recorded the statement after `createUser` returns, before
  the domain user and the link are attempted. Cleanup iterates that list, not
  `set.actors` — the two are now separate concepts, `CreatedAuthUser` for
  identities that exist and `CanaryActor` for actors that are fully configured
  and can sign in.
- Retirement returns the ids the server actually updated (`.select("id")`) and
  compares them as a set against what was requested. The report distinguishes
  `requested`, `matched`, `retired`, `missing` and `duplicates`; a mismatch,
  a duplicate tracked id, or a row that did not come back sets `ok = false` and
  is listed in `problems`. Missing rows are never chased with a wider filter.
- An empty id list is skipped rather than passed to `.in("id", [])`, so an
  unfiltered update cannot be produced by a client-library detail.
- `retire()` is idempotent: the first call's promise is what every later call
  receives, so `create()`'s failure cleanup and a harness `finally` cannot
  double-ban an identity.
- All Supabase access moved behind a narrow `CanaryBackend` with four
  operations — insert, soft-retire, create-auth-user, disable-auth-user. It has
  no delete of any kind, and its retirement primitive takes the id list as a
  required argument, so an unfiltered update is not expressible.
- Ceilings split by what they bound: `MAX_SETUP_DOMAIN_ROWS = 15` (2 branches,
  2 rooms, 5 locations, 1 category, 1 item, 4 users), `MAX_DOCUMENT_ROWS = 5`,
  `MAX_TRACKED_DOMAIN_ROWS = 20`, `MAX_AUTH_USERS = 4`. The plan is unchanged
  and still posts no stock movement and runs no burst.

### 17.6 Tests

`tool/production_canary_namespace_test.ts` — 25 tests, no network, no
credentials. A fake backend fails on demand at each setup step: branch, room,
location, category and item inserts; the first, second, third and fourth
`createUser`; the domain-user insert; the `user_auth_links` insert; the
fourth actor's domain insert; plus retirement mismatch, retirement error, ban
failure and partial cleanup failure. Every case asserts the same three
invariants against what the fake was *asked to do*, not against what the class
reports: no operation outside the four allowed ones, every update carrying a
non-empty id list containing only ids this run created, and every confirmed Auth
identity banned. Idempotency, duplicate detection, ceilings, the
`user_auth_links` left-in-place entries, and the absence of secret material in
the report are covered too.

`tool/production_canary_safety_test.ts` — 11 regression tests reading the source
from disk: no `.delete(`/`deleteUser`/`removeUser`, no
`AISH_ALLOW_DESTRUCTIVE_CLEANUP`, exactly one `.update(` and it carries both
`.in("id", …)` and `.select("id")`, no PostgREST pattern filters in the backend,
the Auth identity tracked between `createUser` and the inserts that follow,
retirement driven by identities rather than actors, every setup write inside the
protected `try`, `retired` counted from confirmed ids, empty id lists skipped,
both harnesses still retiring in `finally`, and the e2e harness tracking all
four purchase-request rows.

### 17.7 Status

| Item | Status |
| --- | --- |
| Live production empty-state recheck, 2026-08-03T14:42:28Z | **PASS** — all 15 relations at 0 |
| Backfill | **SKIP** — justified by the empty-state recheck |
| Approved production preflight, 28/28, commit `14380dc` | **STALE** — the code it checked has changed |
| Production canary | **NOT RUN / BLOCKED** |
| Realtime production canary | **NOT RUN / BLOCKED** |
| Performance / benchmark | **NOT RUN** |
| GO/NO-GO | **HOLD** |

The preflight that passed 28/28 with 0 stop conditions applies to commit
`14380dc` and to nothing else. It checks the working tree and the tooling, both
of which this change modifies, so it must be re-run from the new HEAD. No
production canary result exists, and none of the above may be read as one.

Next operator step: human review of this change, then a fresh production
