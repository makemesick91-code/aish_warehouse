...
class DriftMasterDataRepository implements MasterDataRepository {
  ...
  @override
  Future<List<MasterLocation>> activeRoomLocations(String roomId) async =>
      (await _dao.roomLocations(
        roomId,
      )).map(_toLocation).toList(growable: false);

  @override
  Future<List<MasterLocation>> historicalRoomLocations(String roomId) async =>
      (await _dao.historicalRoomLocations(
        roomId,
      )).map(_toLocation).toList(growable: false);
  ...
  @override
  Future<List<MasterLocation>> activeWarehouseLocations() async =>
      (await _dao.warehouseLocations())
          .map(_toLocation)
          .toList(growable: false);

  @override
  Future<List<MasterLocation>> historicalWarehouseLocations() async =>
      (await _dao.historicalWarehouseLocations())
          .map(_toLocation)
          .toList(growable: false);
