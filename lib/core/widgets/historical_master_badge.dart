...
class HistoricalMasterBadge extends StatelessWidget {
  ...
  /// The header badge, or nothing at all when [detail] names nothing historic.
  ///
  /// One call instead of the `if (usesHistoricalMaster) … detail: …` pair every
  /// document screen would otherwise repeat — the same predicate written out in
  /// eight places is how the eight copies drift apart.
  static Widget forDetail(String detail) => detail.isEmpty
      ? const SizedBox.shrink()
      : HistoricalMasterBadge.historical(detail: detail);
